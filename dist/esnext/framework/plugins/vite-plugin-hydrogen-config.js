        ...
        generateBundle: process.env.WORKER
            ? (options, bundle) => {
                ...
                if (workerBundle.type === 'chunk') {
                    ...
                    workerBundle.code = workerBundle.code.replace(/\((\w+)\.locked\)/gm, '($1 && $1.locked)');
                    ...
                    workerBundle.code = workerBundle.code.replace(/var \w+\s*=\s*(\w+)\.completedRootSegment;/g, 'if($1.status===5)return\n$1.status=5;\n$&');
